...
impl IndexeddbEventCacheStore {
    ...
    ///
    ///     1. `&str` - name of an object store
    ///     2. `&str` - key of an object in the object store
    ///     3. `bool` - whether to encrypt the fields above in the final key
    ///
    /// Selective encryption is employed to maintain ordering, so that range
    /// queries are posisble.
