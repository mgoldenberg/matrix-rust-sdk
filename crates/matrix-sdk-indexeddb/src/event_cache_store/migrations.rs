...
pub async fn open_and_upgrade_db(name: &str) -> Result<Database, OpenDbError> {
    Database::open(name)
        ...
        .with_on_upgrade_needed(|event: VersionChangeEvent, db: Database| -> Result<(), Error> {
            let mut version = Version::try_from(event.old_version() as u32)?;
            while version < current::VERSION {
                version = match version.upgrade(&db)? {
                    Some(next) => next,
...
pub mod v1 {
    ...
    fn create_core_object_store(db: &Database) -> Result<(), Error> {
        let _ = db.create_object_store(keys::CORE).build()?;
        Ok(())
    }
    ...
    fn create_linked_chunks_object_store(db: &Database) -> Result<(), Error> {
        db.create_object_store(keys::LINKED_CHUNKS)
