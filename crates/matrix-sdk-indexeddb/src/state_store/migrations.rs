...
pub async fn upgrade_meta_db(
    ...
    // Meta database.
    let meta_db = Database::open(meta_name)
    ...
    let store_cipher = if let Some(passphrase) = passphrase {
        ...
        let cipher = ob.get(keys::STORE_KEY).serde()?.await?;
        let cipher = if let Some(StoreKeyWrapper(inner)) = cipher {
    ...
    if old_version < CURRENT_DB_VERSION {
        ...
        db = Database::open(name)
            ...
            .with_on_upgrade_needed(
                move |evt: VersionChangeEvent, _: Database| -> Result<(), Error> {
                    // Sanity check.
...
async fn apply_migration(
    ...
    let db = Database::open(name)
        ...
        .with_on_upgrade_needed(move |_: VersionChangeEvent, db: Database| -> Result<(), Error> {
...
async fn backup_v1(source: &Database, meta: &Database) -> Result<()> {
    ...
    let target = Database::open(&backup_name)
    ...
    for name in V1_STORES {
        ...
        for (key, value) in data {
            target_obj.put(value).with_key(key).build()?;
        }
...
async fn migrate_to_v4(db: Database, store_cipher: Option<&StoreCipher>) -> Result<Database> {
    ...
    let sync_token = sync_token_store.get(old_keys::SYNC_TOKEN).build()?.await?;
...
async fn migrate_to_v8(db: Database, store_cipher: Option<&StoreCipher>) -> Result<Database> {
    ...
    // Update the version of the database.
    Ok(Database::open(&name).with_version(8u32).await?)
}
...
async fn migrate_to_v12(db: Database) -> Result<Database> {
    ...
    // Update the version of the database.
    Ok(Database::open(&name).with_version(12u32).await?)
}
...
mod tests {
    ...
    pub async fn test_migrating_v1_to_v2_plain() -> Result<()> {
        ...
        // Create and populate db.
        {
            ...
            custom.put(&serialize_value(None, &CUSTOM_DATA)?).with_key(jskey).build()?;
            tx.commit().await?;
            db.close();
        }
    ...
    pub async fn test_migrating_to_v3() -> Result<()> {
        ...
        // Populate DB with wrong event.
        {
            ...
            state
                .put(&serialize_value(None, &wrong_redacted_state_event)?)
                .with_key(key)
                .build()?;
            tx.commit().await?;
            db.close();
