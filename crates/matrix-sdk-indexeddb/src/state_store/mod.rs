...
use crate::{safe_encode::SafeEncode, GenericError};
...
impl From<IndexeddbStateStoreError> for StoreError {
    fn from(e: IndexeddbStateStoreError) -> Self {
        match e {
            ...
            _ => GenericError::from(e.to_string()).into(),
        }
    }
}
...
impl_state_store!({
    ...
    async fn save_changes(&self, changes: &StateChanges) -> Result<()> {
                    ...
                    for (receipt_type, receipts) in receipts {
                        for (user_id, receipt) in receipts {
                            ...
                            if let Some((old_event, _)) =
                                room_user_receipts.get(&key).build()?.await?.and_then(|f| {
        ...
        if !changes.redactions.is_empty() {
            ...
            for (room_id, redactions) in &changes.redactions {
                ...
                let Some(mut cursor) = state.open_cursor().with_query(&range).build()?.await?
                else {
                    continue;
                };
    ...
    async fn get_state_events(
        ...
        let stripped_events = self
            ...
            .collect::<Result<Vec<_>, _>>()?
            .iter()
            .filter_map(|f| self.deserialize_value(f).ok().map(RawAnySyncOrStrippedState::Stripped))
            .collect::<Vec<_>>();
    ...
    async fn get_room_infos(&self, room_load_settings: &RoomLoadSettings) -> Result<Vec<RoomInfo>> {
        ...
        Ok(match room_load_settings {
            ...
            RoomLoadSettings::One(room_id) => {
                match object_store.get(&self.encode_key(keys::ROOM_INFOS, room_id)).build()?.await?
                {
    ...
    async fn remove_room(&self, room_id: &RoomId) -> Result<()> {
        ...
        for store_name in prefixed_stores {
            let store = tx.object_store(store_name)?;
            let range = self.encode_to_range(store_name, room_id);
            for key in store.get_all_keys::<JsValue>().with_query(&range).build()?.await? {
    ...
    async fn save_send_queue_request(
        ...
        let prev = obj.get(&encoded_key).build()?.await?;
    ...
    async fn remove_send_queue_request(
        ...
        if let Some(val) = obj.get(&encoded_key).build()?.await? {
