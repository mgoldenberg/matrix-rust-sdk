...
impl_crypto_store! {
    async fn save_pending_changes(&self, changes: PendingChanges) -> Result<()> {
        ...
        if let Some(a) = &account_pickle {
            tx.object_store(keys::CORE)?
                .put(&self.serializer.serialize_value(&a)?).with_key(JsValue::from_str(keys::ACCOUNT)).build()?;
        }
    ...
    async fn load_tracked_users(&self) -> Result<Vec<TrackedUser>> {
        let tx = self
            .inner
            .transaction(keys::TRACKED_USERS).with_mode(TransactionMode::Readonly).build()?;
        ...
        for result in user_ids {
            let user_id = result?;
            let dirty: bool =
                !matches!(os.get(&user_id).await?.map(|v: JsValue| v.into_serde()), Some(Ok(false)));
    ...
    async fn load_account(&self) -> Result<Option<Account>> {
        if let Some(pickle) = self
            .inner
            .transaction(keys::CORE).with_mode(TransactionMode::Readonly).build()?
    ...
    async fn get_sessions(&self, sender_key: &str) -> Result<Option<Vec<Session>>> {
        ...
        let sessions: Vec<Session> = self
                .inner
                .transaction(keys::SESSION).with_mode(TransactionMode::Readonly).build()?
    ...
    async fn get_inbound_group_session(
        ...
        if let Some(value) = self
            .inner
            .transaction(keys::INBOUND_GROUP_SESSIONS_V4)
            .with_mode(TransactionMode::Readonly)
            .build()?
    ...
    async fn get_inbound_group_sessions_for_device_batch(
        ...
        let serialized_sessions = idx.get_all().with_query::<Array, _>(key).with_limit(limit as u32).await?;
    ...
    async fn mark_inbound_group_sessions_as_backed_up(&self,
        ...
        for (room_id, session_id) in room_and_session_ids {
            ...
            if let Some(idb_object_js) = object_store.get(&key).await? {
                ...
                object_store.put(&serde_wasm_bindgen::to_value(&idb_object)?).with_key(key).build()?;
            } else {
    ...
    async fn reset_backup_state(&self) -> Result<()> {
        ...
        if let Some(mut cursor) = tx.object_store(keys::INBOUND_GROUP_SESSIONS_V4)?.open_cursor().await? {
            while let Some(value) = cursor.next_record().await? {
                let mut idb_object: InboundGroupSessionIndexedDbObject = serde_wasm_bindgen::from_value(value)?;
                if !idb_object.needs_backup {
    ...
    async fn save_tracked_users(&self, users: &[(&UserId, bool)]) -> Result<()> {
        let tx = self
            .inner
            .transaction(keys::TRACKED_USERS).with_mode(TransactionMode::Readwrite).build()?;
        let os = tx.object_store(keys::TRACKED_USERS)?;
    ...
    async fn delete_secrets_from_inbox(
        ...
        let transaction = self.inner.transaction(keys::SECRETS_INBOX).with_mode(TransactionMode::Readwrite).build()?;
    ...
    async fn delete_outgoing_secret_requests(&self, request_id: &TransactionId) -> Result<()> {
        ...
        let tx = self.inner.transaction(keys::GOSSIP_REQUESTS).with_mode(TransactionMode::Readwrite).build()?;
    ...
    async fn load_backup_keys(&self) -> Result<BackupKeys> {
        let key = {
            let tx = self
                .inner
                .transaction(keys::BACKUP_KEYS).with_mode(TransactionMode::Readonly).build()?;
    ...
    async fn set_custom_value(&self, key: &str, value: Vec<u8>) -> Result<()> {
        let transaction = self
            ...
            .with_mode(TransactionMode::Readwrite)
            .build()?;
        transaction.object_store(keys::CORE)?
            .put(&self.serializer.serialize_value(&value)?).with_key(JsValue::from_str(key)).build()?;
        transaction.commit().await?;
        Ok(())
    }
        ...
        match prev {
            Some(prev) => {
                ...
                if lease.holder == holder || lease.expiration_ts < now_ts {
                    object_store.put(&self.serializer.serialize_value(&Lease { holder: holder.to_owned(), expiration_ts })?).with_key(key).build()?;
                    Ok(true)
                } else {
                    Ok(false)
                }
...
async fn open_meta_db(prefix: &str) -> Result<Database, IndexeddbCryptoStoreError> {
    ...
    Database::open(&name)
        .with_version(1u32)
        .with_on_upgrade_needed(|evt: VersionChangeEvent, db: Database| -> Result<(), Error> {
            let old_version = evt.old_version() as u32;
            if old_version < 1 {
                // migrating to version 1
                db.create_object_store("matrix-sdk-crypto").build()?;
            }
            Ok(())
        })
        .await
